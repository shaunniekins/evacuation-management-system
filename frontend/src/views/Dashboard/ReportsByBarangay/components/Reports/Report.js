...
export const GenderReport = () => {
  const entries = EvacueeReport();
...
export const BarangayReport = () => {
  const evacueeEntries = EvacueeReport();
  const barangayEntries = BarangayReportItem();

  const barangayCounts = {};

  // Initialize barangayCounts object with 0 for each barangay name in barangayEntries
  barangayEntries.forEach((barangay) => {
    barangayCounts[barangay.name] = 0;
  });

  // Count the occurrences of each barangay name in evacueeEntries
  evacueeEntries.forEach((entry) => {
...
export const PWDReport = () => {
  ...
  entries.forEach((entry) => {
    if (entry.is_pwd.toLowerCase() === "yes") {
      pwdNum++;
    } else {
      notPwdNum++;
    }
...
export const AgeReport = () => {
  ...
  entries.forEach((entry) => {
    ...
    const dob = new Date(entry.birthday);
    const age = new Date().getFullYear() - dob.getFullYear();

    // Count entries in each age range
    if (age >= 0 && age <= 5) {
      ageRange["0-5"]++;
